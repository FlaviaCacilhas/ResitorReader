...
#include <opencv2/photo/photo.hpp>
	...
	const int MIN_OBJECT_AREA =  200000;
	const int MAX_OBJECT_AREA = 1500000;
	...
	struct rangeHSV  getColorRange(string color) {
		...
		case brown:
			...
			*/
			// Defining brown color 
			HsvLowAndHigh.iLowH = 2;
			HsvLowAndHigh.iHighH = 8;
			HsvLowAndHigh.iLowS = 100;
			HsvLowAndHigh.iHighS = 255;
			HsvLowAndHigh.iLowV = 20;
			HsvLowAndHigh.iHighV = 200;
			break;
		case red:
			// Defining red color
			HsvLowAndHigh.iLowH = 0;
	...
	int main(int argc, char** argv) {
		...
		//Carregando Imagem
		Mat imgOriginal;
		imgOriginal = imread("c:\\Teste\\teste2.jpg", 1);

		//atenuando ruido da imagem - denoising Image (lento)
		...
		vector<colorFounded> bandFoundedInverse;
		...
		for (int i = 0; i < sixBandColors.size(); i++) {
			...
			if (found.area > MIN_OBJECT_AREA && found.area < MAX_OBJECT_AREA) {
		...
		int vectorSize = bandFounded.size()-1;

		for (int i = 0; i < bandFounded.size(); i++) 
		{
			found = bandFounded[i];
			bandFoundedInverse.push_back(bandFounded[vectorSize - i]);
			if (found.area > MIN_OBJECT_AREA) {
				...
				cout << found.color << "  POS: " << s << " " << found.area << endl;
				circle(points, Point(found.posX, found.posY), 3, Scalar(0, 255, 0), 2);
				putText(points, s, Point(found.posX+5, found.posY), FONT_HERSHEY_COMPLEX_SMALL, 0.6, cvScalar(200, 200, 250), 1, CV_AA);			
			}
		}
		...
		if (bandFounded.size()>3 && bandFounded.size()<6)
		{
			bool showInverse = true;


			if (bandFounded[0].color._Equal("silver") || bandFounded[0].color._Equal("golden"))
			{
				cout << "Resistor Value: " << getResistorValue(bandFoundedInverse) << " " << getResistorPrecision(bandFoundedInverse) << endl;
			}
			else if (bandFounded[vectorSize].color._Equal("silver") || bandFounded[vectorSize].color._Equal("golden"))
			{
				cout << "Resistor Value: " << getResistorValue(bandFounded) << " " << getResistorPrecision(bandFounded) << endl;
			}
			else
			{
				cout << "Resistor Value (normal pos): " << getResistorValue(bandFounded) << " " << getResistorPrecision(bandFounded) << endl;
				cout << endl;
				cout << "Resistor Value (inverse pos): " << getResistorValue(bandFoundedInverse) << " " << getResistorPrecision(bandFoundedInverse) << endl;
			}
				
		}
		else
		{
			cout << "Resistor not property identify." << endl;
		}
